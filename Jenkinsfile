node('linux') {
  ...
  stage('Create Test Stack') {
    ...
    withCredentials([[$class: 'AmazonWebServicesCredentialsBinding'{
	    sh 'aws cloudformation create-stack --template-body file://./docker-single-server.json --stack-name final-test --tags Key=Name,Value=Stack --region us-east-1 --parameters ParameterKey=KeyName,ParameterValue=SEIS665W5 ParameterKey=YourIp,ParameterValue=$(curl ifconfig.me)/32'
